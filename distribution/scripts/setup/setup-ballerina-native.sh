...
function usageCommand() {
    echo "-d <ballerina_installer> -n <netty_host> -G <graalvm_distribution>"
}
...
while getopts "gp:w:o:hd:n:hG" opt; do
    case "${opt}" in
    d)
...
function validate() {
    ...
    if [[ ! -f $graalvm_distribution ]]; then
        echo "Please provide Graal VM distribution."
        exit 1
    fi
