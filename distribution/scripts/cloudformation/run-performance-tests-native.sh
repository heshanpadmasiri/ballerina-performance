...
function usageCommand() {
    echo "-p <ballerina_installer> -G <graalvm distribution> -B <ballerina_ec2_instance_type>"
}
export -f usageCommand

function usageHelp() {
    echo "-i: Ballerina Installer (Debian Package)."
    echo "-G: Graal VM distribution."
    echo "-B: Amazon EC2 Instance Type for Ballerina."
}
export -f usageHelp

while getopts ":u:f:d:k:n:j:o:g:s:b:r:J:S:N:t:p:w:hi:B:G:" opt; do
    case "${opt}" in
    i)
...
function validate() {
    ...
    if [[ ! -f $graalvm_distribution ]]; then
        echo "Please provide the Graal VM distribution."
        exit 1
    fi

    export graalvm_distribution_filename=$(basename $graalvm_distribution)

    if [[ ${graalvm_distribution_filename: -7} != ".tar.gz" ]]; then
        echo "Graal VM distribution must have .tar.gz extension"
        exit 1
    fi
...
function create_links() {
    ...
    graalvm_distribution=$(realpath $graalvm_distribution)
    ln -s $graalvm_distribution $temp_dir/$graalvm_distribution_filename
}
...
function get_cf_parameters() {
    ...
    echo "GraalVmDistribution=$graalvm_distribution_filename"
